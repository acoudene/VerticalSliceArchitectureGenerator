  ...
  where TClient : IRestClient<TDto>
{
    ...
    public virtual async Task<List<TViewObject>> GetAllAsync(Func<TDto, TViewObject> toVoFunc)
    {
        if (toVoFunc is null) throw new ArgumentNullException(nameof(toVoFunc));

        var dtos = await _client.GetAllAsync();
    ...
    public virtual async Task<TViewObject?> GetByIdAsync(Guid id, Func<TDto, TViewObject> toVoFunc)
    {
        if (id == Guid.Empty) throw new ArgumentNullException(nameof(id));
        if (toVoFunc is null) throw new ArgumentNullException(nameof(toVoFunc));

        var entity = await _client.GetByIdAsync(id);
        if (entity is null)
            return null;
    ...
    public virtual async Task<List<TViewObject>> GetByIdsAsync(List<Guid> ids, Func<TDto, TViewObject> toVoFunc)
    {
        if (ids is null) throw new ArgumentNullException(nameof(ids));
        if (!ids.Any()) throw new ArgumentOutOfRangeException(nameof(ids));
        if (toVoFunc is null) throw new ArgumentNullException(nameof(toVoFunc));

        var entities = await _client.GetByIdsAsync(ids);
    ...
    public virtual async Task<TViewObject> CreateAsync(TViewObject newDto, Func<TViewObject, TDto> toDtoFunc)
    {
        ...
        await _client.CreateAsync(toCreateEntity);

        return newDto; // Don't read the inserted value because the Dto should be read and checked in the API.
    }

    public virtual async Task<TViewObject?> UpdateAsync(Guid id, TViewObject updatedDto, Func<TViewObject, TDto> toDtoFunc)
    {
        if (id == Guid.Empty) throw new ArgumentNullException(nameof(id));
        if (id != updatedDto.Id) throw new ArgumentOutOfRangeException(nameof(updatedDto.Id));
        if (toDtoFunc is null) throw new ArgumentNullException(nameof(toDtoFunc));

        var existingEntity = await _client.GetByIdAsync(id);
        if (existingEntity is null)
            return null;

        var toUpdateEntity = toDtoFunc(updatedDto);
        await _client.UpdateAsync(id, toUpdateEntity);

        return updatedDto; // Don't read the updated value because the Dto should be read and checked in the API.
    }

    public virtual async Task<TViewObject?> DeleteAsync(Guid id, Func<TDto, TViewObject> toVoFunc)
    {
        if (id == Guid.Empty) throw new ArgumentNullException(nameof(id));
        if (toVoFunc is null) throw new ArgumentNullException(nameof(toVoFunc));

        var beforeRemoveEntity = await _client.GetByIdAsync(id);
      ...
      Func<TDto, TViewObject> toVoFunc)
    {
        ...
        var existingDto = await _client.GetByIdAsync(id);
