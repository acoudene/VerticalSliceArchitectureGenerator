  ...
  where TClient : IRestClient<TDto>
{
    ...
    public RestBffControllerBase(IHostEnvironment hostEnvironment, ILogger<RestBffControllerBase<TViewObject, TDto, TClient>> logger, TClient client)
    ...
    public virtual async Task<Results<Ok<List<TViewObject>>, BadRequest, ProblemHttpResult>> GetAllAsync()
    {
        try
        {
            return TypedResults.Ok(await _restBffComponent.GetAllAsync(ToViewObject));
        }
    ...
    [HttpGet("{id:guid}")]
    public virtual async Task<Results<Ok<TViewObject>, NotFound, BadRequest, ProblemHttpResult>> GetByIdAsync(Guid id)
    {
        try
        {
            var foundVo = await _restBffComponent.GetByIdAsync(id, ToViewObject);
            if (foundVo is null)
    ...
    [HttpGet("byIds")]
    public virtual async Task<Results<Ok<List<TViewObject>>, BadRequest, ProblemHttpResult>> GetByIdsAsync([FromQuery] List<Guid> ids)
    {
        try
        {
            return TypedResults.Ok(await _restBffComponent.GetByIdsAsync(ids, ToViewObject));
        }
        catch (ArgumentException ex) when (_hostEnvironment.IsDevelopment())
        {
            _logger.LogError(ex, "Bad request");
    ...
    [HttpPost]
    public virtual async Task<Results<Created<TViewObject>, BadRequest, ProblemHttpResult>> CreateAsync([FromBody] TViewObject newVo)
    {
        try
        {
            return TypedResults.Created("{newVo.Id}", await _restBffComponent.CreateAsync(newVo, ToDto));
        }
        catch (ArgumentException ex) when (_hostEnvironment.IsDevelopment())
        {
            _logger.LogError(ex, "Bad request");
        ...
        [FromBody] TViewObject newOrToUpdateVo)
    {
        try
        {
            ...
            return TypedResults.Created("{newOrToUpdateVo.Id}", await _restBffComponent.CreateAsync(newOrToUpdateVo, ToDto));
        }
        catch (ArgumentException ex) when (_hostEnvironment.IsDevelopment())
        {
            _logger.LogError(ex, "Bad request");
    ...
    [HttpPut("{id:guid}")]
    public virtual async Task<Results<NoContent, NotFound, BadRequest, ProblemHttpResult>> UpdateAsync(Guid id, [FromBody] TViewObject toUpdateVo)
    {
        try
        {
            var updatedVo = await _restBffComponent.UpdateAsync(id, toUpdateVo, ToDto);
    ...
    [HttpDelete("{id:guid}")]
    public virtual async Task<Results<Ok<TViewObject>, NotFound, BadRequest, ProblemHttpResult>> DeleteAsync(Guid id)
    {
        try
        {
            var deletedVo = await _restBffComponent.DeleteAsync(id, ToViewObject);
    ...
    [HttpPatch]
    public virtual async Task<Results<Ok<TViewObject>, NotFound, BadRequest, ProblemHttpResult>> PatchAsync(Guid id, [FromBody] JsonPatchDocument<TViewObject> toPatchVo)
    {
        try
        {
            var patchedVo = await _restBffComponent.PatchAsync(id, toPatchVo, ModelState, ToDto, ToViewObject);
